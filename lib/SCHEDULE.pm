...
sub set_schedule {
    ...
    ::print_log( "[SCHEDULE] - DEBUG - set_schedule - Index "
          . $index
          . " Schedule: "
          . $entry
          . " Label "
...
sub get_schedule {
    ...
    my $count;
    my @states;
    my $object = @{ $self->{generic_object} }[0]
      if ( @{ $self->{generic_object} }[0] );
    ...
    for my $index ( 1 .. $count ) {
            ...
            || ( $self->{ 'schedule_once_' . $index } eq 2 ) )
        {
            ...
            if ( defined( $self->{ 'schedule_label_' . $index } ) ) {
                $schedule[ ( $index - $nullcount - $schoncecnt ) ][2] =
                  $self->{ 'schedule_label_' . $index };
            }
            else {
...
sub am_i_active_object {
    ...
    ::print_log(
            "[SCHEDULE] - DEBUG - am_i_active_object - current active object: "
      ...
      if ( ( defined( $Interfaces{$instance} ) )
        && ( $main::Debug{'schedule'} ) );
    if ( defined( $Interfaces{$instance} ) ) {
        if ( $Interfaces{$instance}->get_object_name eq $self->get_object_name )
        {
            return 1;
        }
        else { $self->{'active_object'} = 0; return 0; }
    }
    elsif ( $self->{'active_object'} )
    {    #This is for a restart to get the saved active object.
        my $action = $self->{'active_action'}
          if defined( $self->{'active_action'} );
...
sub _set_instance_active_object {
    ...
    ::print_log( "[SCHEDULE] - Tracking object - "
          . $Tracking_object{$instance}->get_object_name
          . " Active schedule: "
          . $active_schedule_name
          . " Instance: "
          . $instance );
    $Tracking_object{$instance}->set("$active_schedule_name")
      if ( defined( $Tracking_object{$instance} ) );
}

sub _set_instance_active_tracking_object {
    my ( $child, $instance ) = @_;
    ::print_log(
        "Registering a SCHEDULE_Temp Child Object type SCHEDULE_Temp_Active");
...
sub register {
    ...
    if ( $object->isa('SCHEDULE_Generic') ) {
        ::print_log(
            "Registering a SCHEDULE Child Object type SCHEDULE_Generic");
    ...
    if ( $object->isa('SCHEDULE_Temp') ) {
        ...
        if (   ( defined( $self->{temp_object}{'cool'} ) )
            && ( defined( $self->{temp_object}{'heat'} ) ) )
        {
            #::MainLoop_pre_add_hook( sub {SCHEDULE::check_date($self,$self->{temp_object}{'cool'});}, 'persistent' );
            ::MainLoop_pre_add_hook(
                sub {
                    SCHEDULE::check_date( $self, $self->{temp_object}{'cool'} );
                }
            );
        }
    }
...
sub check_date {
    my ( $self, $object ) = @_;
    my $occupied_state = ( $$self{occupied}->state_now )
      if ( defined( $$self{occupied} ) );

    if ($occupied_state) {
        $self->CheckOverRide
          if ( ( $self->am_i_active_object( $$self{instance} ) )
            && ( lc( state $self) eq 'on' ) );
    }
    ...
    if ($::New_Minute) {
        $self->am_i_active_object( $$self{instance} )
        ...
        if ( lc( state $self) eq 'on' ) {
            for my $index ( 1 .. $self->{'schedule_count'} ) {
                if ( defined( $self->{ 'schedule_' . $index } ) ) {
                    if ( &::time_cron( $self->{ 'schedule_' . $index } ) ) {
                        &set_action( $self, $object, $index );
                    }
                }
            }
        }
...
sub setACSetpoint {
    ...
    ::print_log( "[SCHEDULE] running "
          . $cool_temp_control->get_object_name . "->"
    ...
    $self->{'set_temp_timer'}->set(
        ...
        sub {
            ...
            ::print_log( "[SCHEDULE] running "
                  . $heat_temp_control->get_object_name . "->"
                  . $heat_temp_control_sub . "("
                  . $heat_sp
                  . ")" );
        }
    );
...
sub set_action {
    ...
    if ( $object->isa('SCHEDULE_Generic') ) {
        ...
        ::print_log( "[SCHEDULE] Running "
              . $object->{child}->get_object_name . "->"
              . $sub . "("
              . $self->{ 'schedule_label_' . $index }
              . ")" );
        $self->_set_instance_active_object( $$self{instance}, $index )
          if ( defined( $$self{instance} ) );
        $object->{child}->$sub( $self->{ 'schedule_label_' . $index },
            $self->get_object_name, 1 );
    }
    elsif ( $object->isa('SCHEDULE_Temp') ) {
        ::print_log( "[SCHEDULE] - DEBUG - set_action -  Temp object: "
              . $object->get_object_name
              . " Parent object: "
              . $self->get_object_name )
          if $main::Debug{'schedule'};
        ...
        $$self{winter_mode_track_flag} =
          0;    # reset the temp track flag because the schedule changed.
        $self->CheckOverRide;
    }
}

sub set_occpuancy {
    my ( $self, $normal_state, $setback_state, $setback_object, $delay,
        $delay_setback, $tracked_object )
      = @_;
    ...
    $$self{thermo_timer_delay_setback} = $delay_setback
      if ( defined $delay_setback );
...
sub set_vacation {
    ...
    $self->set_occpuancy( undef, undef, undef )
      unless ( defined $$self{occupied} )
      ;    # Allows the use of vacation mode with out occpuancy
}
...
sub CheckOverRide {
    ...
    my $object             = $self;
    my $occupied_state     = ( $$self{occupied}->state )
      if ( defined( $$self{occupied} ) );

    if ( $self->OverRide ) {
        $occ_setback_object = $$self{override_mode_setback_object}
          if defined( $$self{override_mode_setback_object} );
        $occ_setback_state = $$self{override_mode_setback_state}
          if defined( $$self{override_mode_setback_state} );
        $occ_state = $$self{override_mode_occ_state}
          if defined( $$self{override_mode_occ_state} );
        $object = $$self{override_mode_object}
          if defined( $$self{override_mode_object} );
    }
    elsif ( $checktype eq 'temp_track' ) { return }
    elsif ( $$self{winter_mode_track_flag} ) {
        $object = $$self{winter_mode_object};
    }
    ...
    if (   ( defined( $$self{occupied} ) )
        && ( $$self{occupied}->state eq $occ_state ) )
    {
        if ( $$self{thermo_timer}->expired ) {
            ::print_log( "[SCHEDULE] - INFO - setting "
                  . $object->get_object_name
                  . " setpoints, you are now $occ_state" );
    ...
    elsif (( defined( $$self{occupied} ) )
        && ( $$self{occupied}->state eq $occ_setback_state ) )
    {
        if ( $$self{thermo_timer}->expired ) {
            ::print_log( "[SCHEDULE] - INFO - setting setback "
                  . $occ_setback_object->get_object_name
                  . " setpoints, you are now $occ_setback_state" );
...
sub OverRide {
    ...
    ::print_log("[SCHEDULE] - DEBUG --- IN OVERRIDE")
      if $main::Debug{'schedule'};
    if ( $occupied_state eq $$self{vacation_mode_state} ) {
        ::print_log("[SCHEDULE] - DEBUG --- IN OVERRIDE --- VACATION")
          if $main::Debug{'schedule'};
        $$self{override_mode_object} = $$self{vacation_mode_object}
          ;    # override the setpoint if in vacation mode
        return 1;
    }
...
sub WinterMode {
    ...
    my ($self) = @_;
    ::print_log("[SCHEDULE] - DEBUG --- IN WINTERMODE")
      if $main::Debug{'schedule'};
    if (   ( $$self{winter_mode_type} eq 'night' )
        && ( $::Weather{'Forecast Tonight'} =~ /lows in the ([\w ]+) (\d+)/i ) )
    {
        ::print_log(
            "[SCHEDULE] - DEBUG --- IN WINTERMODE --- FORCAST --- $1 $2")
          if $main::Debug{'schedule'};
        my $fc = $2;
        if ( lc($1) =~ /mid/ ) {
            $fc = $fc + 3;
        }    # Translate low, mid, and upper to a value
        if ( lc($1) =~ /up/ ) { $fc = $fc + 6 }
        ...
        if ( $fc <= ( $$self{winter_mode_temp} ) ) {
            ::print_log(
                "[SCHEDULE] - DEBUG --- IN WINTERMODE  ---- M1 --- LOWS -- $fc -- $$self{winter_mode_temp}"
            ) if $main::Debug{'schedule'};
            $$self{override_mode_object} = $$self{winter_mode_object}
              ;    # override the setpoint if forcast temp is below config
            return 1;
        }
        return 0;
    }
      ...
      )
    {
        ...
        if    ( lc($1) =~ /around/ ) { $fc = $fc + 0 }
            ...
            ::print_log(
                "[SCHEDULE] - NOTICE --- WINTERMODE --- Unknown forecast modifier: $1 -- full text: $::Weather{'Forecast Today'}"
            );
        }
        ##if the value we got from the weather script is equal
        #or lower than our defined value, return the defined winter mode
    ...
    if (   ( $$self{winter_mode_type} eq 'now' )
        && ( $::Weather{'TempOutdoor'} =~ /(\d+)/i ) )
    {
        ::print_log("[SCHEDULE] - DEBUG --- IN WINTERMODE --- TEMP NOW --- $1")
          if $main::Debug{'schedule'};
        my $fc = $1;
        if ( $fc <= ( $$self{winter_mode_temp} ) ) {
            ::print_log(
                "[SCHEDULE] - DEBUG --- IN WINTERMODE  --- TEMP NOW -- $fc -- $$self{winter_mode_temp}"
            ) if $main::Debug{'schedule'};
            $$self{override_mode_object} = $$self{winter_mode_object}
    ...
    if (   ( $$self{winter_mode_type} eq 'track' )
        && ( $::Weather{'TempOutdoor'} =~ /(\d+)/i ) )
    {
        ::print_log(
        ...
        if (   ( $fc <= ( $$self{winter_mode_temp} ) ) && ( not $$self{winter_mode_track_flag} ) )
        {
            ...
            ::print_log(
                "[SCHEDULE] - DEBUG --- IN WINTERMODE  --- TEMP TRACK -- TempOutdoor: $fc -- Config low: $$self{winter_mode_temp}"
            ) if $main::Debug{'schedule'};
            $$self{override_mode_object} = $$self{winter_mode_object}
              ;    # override the setpoint if current temp is below config
            return 1;
        ...
        elsif (( $fc > ( $$self{winter_mode_temp_high} ) ) && ( $$self{winter_mode_track_flag} ) )
        {
            ...
            ::print_log(
                "[SCHEDULE] - DEBUG --- IN WINTERMODE  --- TEMP TRACK -- TempOutdoor: $fc -- Config high: $$self{winter_mode_temp}"
            ) if $main::Debug{'schedule'};
            $$self{override_mode_object} = $self
              ;  # set the setpoint back to normal if temp is higher than config
            return 1;
        }
        return 0;
    }
